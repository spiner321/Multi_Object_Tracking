...
def make_img_lidar(img_pth, out_pth, projected_arr, d, show=False):
    ...
    plt.savefig(f'{out_pth}bg_overlay_lidar.jpg', bbox_inches='tight', pad_inches=0)
    if show: plt.show()
...
def make_img_cube(img_pth, out_dir, projected_cubes):
    ...
    for cube in projected_cubes:
        min_xy = np.min(cube, axis=0).squeeze()
        max_xy = np.max(cube, axis=0).squeeze()
        
        org = ( int(min_xy[0]), int(max_xy[1])+size[1] )
        
        
        cv2.rectangle(image, org, (org[0]+size[0], org[1]-size[1]), (255,0,0), -1)
        ...
        plt.scatter(x=u, y=v, color='red', alpha=1.0, s=0.5)
        plt.imshow(np.array(image))
