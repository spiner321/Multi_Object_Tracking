...
def show_lidar(lidar_pth):
    ...
    print(f"lidar point cloud number: {len(lidar_pcd_arr)}")
...
def show_fusion(lidar_pth, radar_pth, conf_dir=None, rot_dscr_radar=None, to_calib=False):
    ...
    if to_calib:
        ...
        radar_pcd_arr = calib_logic.rotate_translate_pcd(radar_pcd_arr, rmat_radar, tvec_radar)
...
def show_cuboid(lidar_pth, cube_list):
    lidar_pcd_arr = calib_parser.parse_pcd(lidar_pth)
    pcd_arr = lidar_pcd_arr
    ...
    for cube in cube_list:
        ...
        cube_color = np.zeros((len(cube_arr), 3))
        cube_color = pd.DataFrame(cube_color, columns=['r', 'g', 'b'], dtype=np.int64)
        cube_color['r'] = cube_color['r'] + 255
        cube_color = cube_color.to_numpy()
                
        pcd_arr = np.concatenate((pcd_arr, cube_arr), axis=0)
        color = np.concatenate((color, cube_color), axis=0)

                
        pass

    
    # X, Y, Z = vis.get_cuboid_arr(center, size)
    # cube_arr = np.concatenate((X, Y, Z), axis=0)
    # cube_color = np.zeros((len(cube_arr), 3))
    # cube_color = pd.DataFrame(cube_color, columns=['r', 'g', 'b'], dtype=np.int64)
    # cube_color['r'] = cube_color['r'] + 255
    ...
    point_cloud = o3d.geometry.PointCloud()
    point_cloud.points = o3d.utility.Vector3dVector(pcd_arr) # array_of_points.shape = (N,3)
    point_cloud.colors = o3d.utility.Vector3dVector(color) # array_of_colors.shape = (N,3)
