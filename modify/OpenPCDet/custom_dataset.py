...
class CustomDataset(DatasetTemplate):
    def __init__(self, dataset_cfg, class_names, training=True, root_path=None, logger=None, mode=None):
        ...
        """
        super().__init__(
            dataset_cfg=dataset_cfg, class_names=class_names, training=training, root_path=root_path, logger=logger, mode=mode
        )
...
def create_custom_infos(dataset_cfg, class_names, data_path, save_path, workers=4):
    # data_path = '/data/NIA50/data/특수환경 자율주행 3D 이미지/mm_Train3D'
    ...
    print('Custom info train file is save to %s' % test_filename)
