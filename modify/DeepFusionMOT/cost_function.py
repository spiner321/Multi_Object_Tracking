...
def poly_area(x, y):
    ...
    # (Pius)Fix a performance issue with numba. 
    # Issue message: NumbaPerformanceWarning: np.dot() is faster on contiguous arrays, called on (array(float64, 1d, A), array(float64, 1d, C))
    x_contig = np.ascontiguousarray(x)
    y_contig = np.ascontiguousarray(y)
    return 0.5 * np.abs(np.dot(x_contig, np.roll(y_contig, 1)) - np.dot(y_contig, np.roll(x_contig, 1)))
